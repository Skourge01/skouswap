...
desativar_zram() {
    if grep -q "/dev/zram0" /proc/swaps; then
        ...
        sleep 2  # Adicionando um delay adicional para garantir que a memória seja completamente liberada
...
zram_file() {
    ...
    echo "2. Aitvar ZRAM 100% da RAM" 
    echo "3. Ativar ZRAM 75% da RAM" 
    echo "4. Desativar ZRAM"
    echo "5. Voltar"
	...
	2) 
	   clear
            ...
            echo "Configurando ZRAM..."
            total_ram_kb=$(grep MemTotal /proc/meminfo | awk '{print $2}')
            zram_size=$((total_ram_kb * 1024)) # 100% da RAM
	...
	clear
            ...
            echo "Configurando ZRAM..."
            total_ram_kb=$(grep MemTotal /proc/meminfo | awk '{print $2}')
            zram_size=$((total_ram_kb * 768)) # 75% da RAM
